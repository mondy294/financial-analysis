...
4. **务必结合近 N 日日线明细判断走势形态**：单日指标压不出的信息要从原始 OHLC 里读，例如高开低走/冲高回落（开在高位但收在低位、上影线长）、连续放量拉升或缩量阴跌、量价背离、跳空缺口等，并纳入技术面判断。
5. 语言精炼，**总长不超过 280 字**。
6. 使用 Markdown 格式，包含以下 4 个小节：
   - **技术面**：均线/动量/量能的组合解读 + **近 N 日走势形态**（结合共振度评估信号强度）
   - **基本面**：估值 & 增长的对比（如果有数据）
   - **风险提示**：**逐条呼应规则系统标记的 risk_flags**，并加上自己从走势/指标里发现的其它风险
   - **观察建议**：可跟踪的量化指标或价位阈值
7. 如果某类数据缺失（NULL），就跳过对应的判断，**不要说"数据缺失"这种废话**。
