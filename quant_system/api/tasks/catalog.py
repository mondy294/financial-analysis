...
TASK_CATALOG: dict[str, TaskSpec] = {
    ...
    "pattern.scan": TaskSpec(
        ...
        group="analysis",
        label="Pattern 正式扫描",
        description="仅 published Definition；写入 abnormal_signal。可填起止日期批量扫。",
        params=(
            _DATE,
            TaskParam(
                "start_date",
                "date",
                "开始日",
                help="与结束日一起构成区间；空则用交易日",
            ),
            TaskParam(
                "end_date",
                "date",
                "结束日",
                help="与开始日一起构成区间；空则用交易日",
            ),
    ...
    "eea.build_panel": TaskSpec(
        id="eea.build_panel",
        group="analysis",
        label="业绩事件 Panel 构建",
        description="拉取披露事件并构建 Earnings Event Panel（含特征与 5/10/20 日收益）",
        heavy=True,
        params=(
            TaskParam("start_date", "date", "开始日", required=True),
            TaskParam("end_date", "date", "结束日", required=True),
            TaskParam("panel_tag", "string", "Panel 标签", default="default"),
            TaskParam("build_events", "bool", "先构建事件", default=True),
            TaskParam("main_only", "bool", "仅主板", default=True),
            TaskParam("cluster_run_id", "string", "簇 run_id", help="空=最新成功 run"),
        ),
    ),
    "eea.fit": TaskSpec(
        id="eea.fit",
        group="analysis",
        label="业绩事件模型拟合",
        description="按综合/中报/年报 × 分簇模式拟合 OLS + Median EY",
        heavy=True,
        params=(
            TaskParam("panel_tag", "string", "Panel 标签", default="default"),
            TaskParam(
                ...
                "string",
                "拟合宇宙",
                default="all,interim,annual",
                help="逗号分隔：all,interim,annual",
            ),
            TaskParam(
                "cluster_modes",
                "string",
                "分簇模式",
                default="none,fixed_effect,per_cluster",
                help="逗号分隔：none,fixed_effect,per_cluster",
            ),
        ),
    ),
    # ---- 运维 ----
