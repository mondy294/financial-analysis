...
```bash
qs serve --dev               # 后端 reload + Vite 前端 HMR
# API  → http://127.0.0.1:8000
# 前端 → http://127.0.0.1:5173   ← 开发请开这个地址
```

仅后端热重载：`qs serve --reload`。生产静态包：`cd web && npm run build && qs serve`。
...
| `qs serve [--dev]` | Web Console：`--dev` = 后端热重载 + Vite 前端 HMR |
...
示例：跑 **2026-07-16** 的关系分析（需已有当日及足够历史 K 线）：

```bash
# 1) 确保数据就绪（有 K 线即可；池用 .env 里的 STOCK_POOL）
qs update kline --date 2026-07-16

# 2) 先 dry-run 看阈值是否合适，再正式落库（W60 + W250）
qs relationship build --date 2026-07-16 --windows 60,250 --dry-run
qs relationship build --date 2026-07-16 --windows 60,250 --force

# 3) 查结果
qs relationship stats
qs relationship strong --date 2026-07-16 --window 250 --min-abs 0.8 --limit 20
qs relationship top 600519.SH --date 2026-07-16 --window 250 --limit 15
qs relationship pair 600519.SH 000858.SZ --date 2026-07-16 --window 250
qs relationship changed --date 2026-07-16 --short 60 --long 250 --min-delta 0.3

# 4) 可选：领先-滞后（依赖上一步 PEARSON 快照）
qs relationship leadlag --date 2026-07-16 --window 60 --force
qs relationship leads 600519.SH --date 2026-07-16 --window 60 --limit 10
```
...
| 命令 | 说明 |
|---|---|
| `qs abnormal scan [--date --patterns --dry-run --force]` | 形态模板匹配扫描（Definition 驱动），命中后按 similarity 排名落库 |
...
| `qs abnormal stats [--date]` | 各 Pattern 命中数 |

扫描并发由环境变量控制：`QS_PATTERN__CONCURRENCY`（默认 4，设为 1 关闭并发）。
